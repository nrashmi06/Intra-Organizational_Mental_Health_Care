...
export const getAverageSessionDetails = async (token: string) => {
  try {
    const response = await axiosInstance.get(SESSION_API_ENDPOINTS.AVG_SESSION_DURATION, {
      headers: {
    ...
    console.log("Average session summary:", response);
      ...
      if (isAxiosError(error)) {
        console.error("Error fetching session summary:", (error.response?.data as string) || error.message);
      } else {
        console.error("Error fetching session summary:", (error as Error).message);
      }
